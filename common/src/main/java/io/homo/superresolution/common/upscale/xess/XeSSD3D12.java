...
/**
 * Intel XeSS Super Resolution through the D3D12 backend, driven by the GL↔D3D12
 * interop path. Mirrors {@code FfxFSR4D3D12} but uses the XeSS provider
 * ({@code libxess.dll}) instead of the AMD FFX upscaler.
 */
public final class XeSSD3D12
        extends D3D12InteropAlgorithm<SRUpscaleContext> {
    public static final String XESS_DLL_NAME = "libxess.dll";
    private static final long PROVIDER_ID = 0x8000004L;
    ...
    protected SRUpscaleContext createD3D12Upscaler(
            ...
            InteropSize size) {
        Path providerLibrary = NativeLibManager.LIB_SUPER_RESOLUTION_XESS
                .getTargetPath(SuperResolutionConstants.NATIVE_LIBRARIES_DIR.getPath())
                .toAbsolutePath();
        Path xessDll = SuperResolutionConstants.NATIVE_LIBRARIES_DIR
                .getPath()
                .resolve(XESS_DLL_NAME)
                .toAbsolutePath();
        if (!Files.isReadable(providerLibrary)) {
            throw new IllegalStateException(
                    "XeSS provider library is missing: " + providerLibrary);
        }
        if (!Files.isReadable(xessDll)) {
            throw new IllegalStateException(
                    "XeSS DLL is missing: " + xessDll);
        }
        ...
        if (loadCode != SRReturnCode.OK) {
            throw new IllegalStateException(
                    "Could not load XeSS providers: " + loadCode);
        }
        ...
        try (SRUpscaleProvider provider = new SRUpscaleProvider(0)) {
            ...
            if (providerCode != SRReturnCode.OK) {
                throw new IllegalStateException(
                        "Could not acquire the D3D12 XeSS provider: " +
                                providerCode);
            }
            ...
            try (SRCreateUpscaleContextDesc createDesc =
                ...
                SRReturnCode pathCode = createDesc
                        .getExtraParams()
                        .setString("XESS_DLL_PATH", xessDll.toString());
                if (pathCode != SRReturnCode.OK) {
                    throw new IllegalStateException(
                            "Could not configure the XeSS DLL path: " +
                                    pathCode);
                }
                ...
                if (createCode != SRReturnCode.OK) {
                    throw new IllegalStateException(
                            "Could not create the XeSS D3D12 context: " +
                                    createCode);
                }
                ...
                if (initCode != SRReturnCode.OK) {
                    context.destroy();
                    throw new IllegalStateException(
                            "Could not initialize the XeSS D3D12 context: " +
                                    initCode);
                }
            }
            return context;
        ...
        if (context.nativePtr > 0) {
            ...
            if (code != SRReturnCode.OK) {
                SuperResolution.LOGGER.error(
                        "Failed to destroy XeSS D3D12 context: {}",
                        code);
            }
        }
    }
    ...
    protected boolean dispatchD3D12Upscale(
        ...
        try (SRDispatchUpscaleDesc desc = new SRDispatchUpscaleDesc()) {
            ...
            // XeSS D3D12 requires the output in UNORDERED_ACCESS state.
