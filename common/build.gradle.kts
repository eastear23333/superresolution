...
plugins {
    `java-library`
    // Windows API FFM bindings (D3D12 presentation, Java 25+). Declared here so the
    // plugin is on the buildscript classpath; applied conditionally below.
    id("net.codecrete.windows-api") version "0.8.6" apply false
}
...
// D3D12 presentation (Java 25+): FFM bindings generated by WindowsApiGenerator.
// The plugin itself is toolchain-agnostic; the generated code needs a Java 25
// toolchain, so the generation task is only configured for javaVersion >= 25.
val d3d12Enabled = versionConfig.common.javaVersion >= 25

// Keep the Java toolchain aligned with the configuration's java_version (e.g. 25
// for 1.21.11) so the FFM bindings compile with a matching compiler.
java {
    toolchain {
...
if (d3d12Enabled) {
    tasks.named("generateWindowsApi") {
        withGroovyBuilder {
            setProperty("functions", listOf("D3D12CreateDevice", "CreateDXGIFactory2", "CreateDXGIFactory1"))
            setProperty("comInterfaces", listOf(
                "ID3D12Device", "IDXGIFactory6", "IDXGIAdapter1", "IDXGISwapChain3",
